...
public class ClockView extends View {
    ...
    private static final float SECOND_NEEDLE_LENGTH_RATIO = 0.68f; // 秒针长度相对表盘半径的比例
    private static final float HOUR_NEEDLE_WIDTH = 16; // 时针的宽度
    private static final float MINUTE_NEEDLE_WIDTH = 12; // 分针的宽度
    private static final float SECOND_NEEDLE_WIDTH = 8; // 秒针的宽度
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        double minuteDegree = (minute-15)*6+second*0.1;
        double minuteRadian = Math.toRadians(minuteDegree);
        double secondDegree = (second-15)*6;
        double secondRadian = Math.toRadians(secondDegree);
        float endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(hourRadian));
        float endY= (float)(centerY+radius*HOUR_NEEDLE_LENGTH_RATIO* Math.sin(hourRadian));
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            canvas.drawText(
                    number,
                    (float) (centerX + (radius * 0.77f + textWidth / 2f) * Math.cos(Math.toRadians(hourDegree))),
                    (float) (centerY + 35 + (radius * 0.77f + textHeight / 2f) * Math.sin(Math.toRadians(hourDegree))),
                    numberPaint
            );
        }
    }
